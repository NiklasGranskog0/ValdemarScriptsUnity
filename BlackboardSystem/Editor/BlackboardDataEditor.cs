...
namespace Assets.Scripts.Framework.BlackboardSystem.Editor
{
    [CustomEditor(typeof(BlackboardData))]
    public class BlackboardDataEditor : UnityEditor.Editor
    {
        ReorderableList m_EntryList;

        void OnEnable()
        {
            m_EntryList = new ReorderableList(serializedObject, serializedObject.FindProperty("entries"), true, true,
                true, true)
            {
            ...
            m_EntryList.drawElementCallback = (rect, index, isActive, isFocused) =>
            {
                var element = m_EntryList.serializedProperty.GetArrayElementAtIndex(index);
